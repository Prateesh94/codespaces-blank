...
func conn() {
	...
	qry := `use blogs`
	db.Exec(qry)
	qry = `create table if not exists myblogs(id int not null primary key auto_increment,title text,content text,category text,tags text,created timestamp default current_timestamp,updated timestamp default current_timestamp)`
	db.Exec(qry)
	fmt.Println("tried to create table")
}
